...
import programs.nansen_rapid_research_v1.runner as runner
...
from programs.nansen_rapid_research_v1.evidence import (
...
def activated_program(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(runner, "require_stopped_v1_activation", lambda _: program)
...
def _reconciliation() -> dict[str, Any]:
    return {
        ...
        "owner_aborted_v1_attestation": {
            "path": "activation/owner-aborted-v1-attestation.json",
            "sha256": "a" * 64,
            "operational_replay_sha256": "b" * 64,
        },
        "operator_stop_attestation": {
            "path": "activation/operator-stop-attestation.json",
            "sha256": "c" * 64,
        },
        "operational_ledgers": [
            {
                ...
                "operational_ledger_sha256": "3132f1bfaa5e99d535bd6ded819f9751a44bede2f6dbf0bf60689cd0c9c49230",
            ...
            },
            {
                ...
                "terminal_stage": "unscorable",
                "operational_ledger_sha256": "5f58ce65563be7f4ab909b3b00fa2bbac5eba590d9b534f8216b14043181d230",
                "confirmed_spend_credits": 4_828,
                "reserved_spend_candidates": [0, 1],
            },
            {
                "program_id": "2026-08-18-prospective-multi-cycle-cohort-v1",
...
def _terminalize_cycles(program: ParallelStrategyProgram, cycles) -> None:
    for cycle in cycles:
        runner._terminalize(
            ...
            cycle,
            "missed_start_window",
            clock=lambda cycle=cycle: cycle.scheduled_at
...
def test_orphan_terminal_seal_is_adopted_after_state_update_crash(
    ...
    assert adopted["terminal_reason"] == "missed_start_window"
...
def test_unscorable_reason_must_replay_from_frozen_timing_or_evidence(
    activated_program,
):
    program, _ = activated_program
    cycle = SCHEDULE[0]
    runner._terminalize(
        program,
        cycle,
        "operator_claimed_unavailable",
        clock=lambda: cycle.scheduled_at + timedelta(minutes=16),
    )

    with pytest.raises(
        runner.ParallelStrategyRunnerError, match="reason is not frozen"
    ):
        runner.check_cycle(program, cycle.index)


def test_missed_start_reason_cannot_be_sealed_before_the_deadline(
    activated_program,
):
    program, _ = activated_program
    cycle = SCHEDULE[0]
    ...
    with pytest.raises(
        runner.ParallelStrategyRunnerError, match="does not replay from timing"
    ):
        runner.check_cycle(program, cycle.index)


def test_program_fatal_replay_rejects_any_later_cycle_activity(
    activated_program,
):
    program, manifest_path = activated_program
    fatal_cycle, later_cycle = SCHEDULE[:2]
    runner._program_fatal(
        program,
        fatal_cycle,
        "fixture_contract_failure",
        clock=lambda: fatal_cycle.scheduled_at + timedelta(minutes=5),
    )
    runner._terminalize(
        program,
        later_cycle,
        "missed_start_window",
        clock=lambda: later_cycle.scheduled_at
        + timedelta(minutes=15, microseconds=1),
    )

    with pytest.raises(
        runner.ParallelStrategyRunnerError,
        match="activity exists after the global fatal cycle",
    ):
