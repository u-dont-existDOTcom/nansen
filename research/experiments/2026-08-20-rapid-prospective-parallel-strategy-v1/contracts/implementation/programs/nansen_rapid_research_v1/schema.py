...
class ParallelStrategySchemaError(RuntimeError):
    """Raised when the frozen rapid-research program cannot be trusted."""
...
def _source_paths(repo_root: Path) -> tuple[Path, ...]:
    package = repo_root / "programs/nansen_rapid_research_v1"
    if package.is_symlink():
        raise ParallelStrategySchemaError("rapid-research package cannot be a symlink")
    ...
    if scripts.is_symlink():
        raise ParallelStrategySchemaError("rapid-research scripts root cannot be a symlink")
    if scripts.is_dir():
        paths.extend(sorted(scripts.glob("*rapid_research*.py")))
    units = repo_root / "operations"
    if units.is_symlink():
        raise ParallelStrategySchemaError("rapid-research units root cannot be a symlink")
    if units.is_dir():
        paths.extend(sorted(units.glob("nansen-signal-lab-rapid-research*")))
    tests = repo_root / "tests"
    if tests.is_symlink():
        raise ParallelStrategySchemaError("rapid-research tests root cannot be a symlink")
    if tests.is_dir():
        paths.extend(sorted(tests.glob("test_rapid_research*.py")))
    ...
    if not required_names <= observed_names:
        ...
        raise ParallelStrategySchemaError(
            f"rapid-research source closure is incomplete: {missing[0]}"
        )
    if not any(path.parent == tests for path in paths):
        raise ParallelStrategySchemaError("rapid-research tests are unavailable")
...
def _runtime_document(repo_root: Path, program_root: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "kind": "rapid-research-runtime-freeze-v1",
        "program_id": PROGRAM_ID,
...
def _preregistration_text() -> bytes:
    return (
        ...
        "- The exact owner-aborted cohort-v1 cycle-one state and stopped legacy "
        "automation are required before the first action.\n"
...
def initialize_program(
    ...
    *,
    ...
    if root.exists() or root.is_symlink():
        raise FileExistsError(f"refusing to reuse rapid-research root: {root}")
    ...
    v1_program = _regular_file(
        _confined(
            ...
            field="owner-aborted v1 prerequisite path",
        ),
        label="owner-aborted v1 prerequisite",
    )
    ...
    manifest = {
        "schema_version": 1,
        "kind": "rapid-research-program-v1",
        "program_id": PROGRAM_ID,
...
def _validate_runtime(program: ParallelStrategyProgram, *, live: bool) -> dict[str, Any]:
    ...
    if (
        ...
        or runtime.get("kind") != "rapid-research-runtime-freeze-v1"
...
def load_program(
    ...
    *,
    ...
    manifest = _read_object(path, label="rapid-research manifest")
    expected_keys = {
        "schema_version",
        "kind",
        "program_id",
    ...
    if set(manifest) != expected_keys:
        raise ParallelStrategySchemaError("rapid-research manifest keys differ")
    if (
        manifest.get("schema_version") != 1
        or manifest.get("kind") != "rapid-research-program-v1"
        ...
        raise ParallelStrategySchemaError("rapid-research manifest identity differs")
    created = _parse_utc(manifest.get("created_at"), field="created_at")
    if created >= SCHEDULE[0].scheduled_at:
        raise ParallelStrategySchemaError("rapid-research creation time differs")
    if manifest.get("budget") != {
        **budget_contract(),
        "maximum_authenticated_attempts": MAX_PROGRAM_ATTEMPTS,
        "maximum_billable_credits": MAX_PROGRAM_CREDITS,
    }:
        raise ParallelStrategySchemaError("rapid-research budget differs")
    ...
    if (
        ...
        or prerequisite.get("kind") != "owner-aborted-v1-operational-attestation-v1"
        ...
        raise ParallelStrategySchemaError("owner-aborted v1 prerequisite differs")
    v1_path = _regular_file(
        _confined(
            repo_root,
            V1_PROGRAM_RELATIVE_PATH.as_posix(),
            field="owner-aborted v1 program path",
        ),
        label="owner-aborted v1 program",
    )
    if sha256_file(v1_path) != prerequisite["program_sha256"]:
        raise ParallelStrategySchemaError("owner-aborted v1 program manifest drifted")
    ...
    if manifest["stage"] == "activated":
        ...
        from .runtime import validate_stopped_v1_activation
...
def replay_program(manifest_path: Path) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "maximum_authenticated_attempts": MAX_PROGRAM_ATTEMPTS,
        "maximum_billable_credits": MAX_PROGRAM_CREDITS,
        "owner_abort_attested": program.stage == "activated",
    }
